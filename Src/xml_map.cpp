...
bool XMLMap::PrepareMap(const char *FileName)
{
    ...
    for (mapnode = map->FirstChildElement(); mapnode; mapnode = mapnode->NextSiblingElement()) {
        ...
        if (value == CNS_TAG_HEIGHT) {
            if (hasHeight) {
                ...
                std::cout << "Only first value of '" << CNS_TAG_HEIGHT << "' =" << height_ << "will be used."
                          << std::endl;
            }
            else {
                if (!((stream >> height_) && (height_ > 0))) {
            ...
            if (hasWidth) {
                ...
                std::cout << "Only first value of '" << CNS_TAG_WIDTH << "' =" << width_ << "will be used." << std::endl;
            }
            else {
                if (!((stream >> width_) && (width_ > 0))) {
            ...
            if (hasCellSize) {
                ...
                std::cout << "Only first value of '" << CNS_TAG_CELLSIZE << "' =" << cell_size_ << "will be used."
                          << std::endl;
            }
            else {
                if (!((stream >> cell_size_) && (cell_size_ > 0))) {
            ...
            if (hasFINX) {
                ...
                if (!(stream >> goal_j && goal_j >= 0 && goal_j < width_)) {
            ...
            if (hasFINY) {
                ...
                if (!(stream >> goal_i && goal_i >= 0 && goal_i < height_)) {
            ...
            while (grid_i < height_) {
                ...
                int val;
                if (elems.size() > 0)
                    for (grid_j = 0; grid_j < width_; ++grid_j) {
    ...
    if (grid_[start_i][start_j] != CN_GC_NOOBS) {
        std::cout << "Error! Start cell is not traversable (cell's value is" << grid_[start_i][start_j] << ")!"
                  << std::endl;
        return false;
    }

    if (grid_[goal_i][goal_j] != CN_GC_NOOBS) {
        std::cout << "Error! Goal cell is not traversable (cell's value is" << grid_[goal_i][goal_j] << ")!"
                  << std::endl;
        return false;
    }
...
int XMLMap::getValue(int i, int j) const
{
    if (i < 0 || i >= height_)
        return -1;
...
{
    height_ = orig.GetHeight();
    width_ = orig.GetWidth();
